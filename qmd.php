...
if(preg_match('~data-aria-label-part="0">(.*)</p>~',$page_content,$res)){
    echo $res[1];
};
//echo $res[1];
...
while(1){
	...
	if($min==10){
		$url = 'https://twitter.com/LittleAprilFool';
		$page_content = file_get_contents($url);
		preg_match('~data-aria-label-part="0">(.*)</p>~',$page_content,$res);
		echo $res[1];	
	}
	...
	// save the image
	imagejpeg($dest, $dic.'april_qmd.jpg',100);
